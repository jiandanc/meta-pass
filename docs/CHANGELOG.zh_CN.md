...
- 子固件息屏后按键唤醒可回到该固件继续运行（2026-09-24）。槽位固件自行按空闲超时
  （如 60 秒）进 deep sleep 后，按键唤醒对 bootloader 而言是一次完整启动：该槽位的
  otadata 副本停在 `PENDING_VERIFY`，回滚逻辑会先把它标成 `ABORTED`，两个副本都无
  候选 → 回退 factory 启动器。结果不是"续玩"，而是子固件当场退出、启动器接管。修复
  分两处：

  （1）bootloader 钩子改为按复位原因分流。深睡唤醒时，把正在运行的子固件那条
  `PENDING_VERIFY` 副本续期为 `VALID`——先复核 CRC、再擦除扇区后写入（flash 只能把 1
  写成 0，0x1 → 0x2 必须擦后重写）——bootloader 随即正常引导回该槽位。`PENDING_VERIFY`
  只可能由 bootloader 在选择某个 OTA 槽时写入，故"读到 PENDING"等价于"上次引导的是子
  固件且它尚未确认"。冷复位（断电/看门狗/崩溃/软件重启）仍走原来的 `VALID` 擦除策略、
  仍回启动器列表页，单次会话模型不变；续期写出的 `VALID` 会在下一次冷启动被擦掉。

  为什么由启动器侧改写 otadata：IDF 的 `CONFIG_BOOTLOADER_SKIP_VALIDATE_IN_DEEP_SLEEP`
  先试过，真机证据否决——它把"上次引导的分区"记在 RTC 快速内存顶部的
  `rtc_retain_mem_t`，而链接脚本只在 `CONFIG_BOOTLOADER_RESERVE_RTC_MEM` 打开时才为该
  记录预留空间。子固件的构建没开这一项（它的 RTC 定时器数据正好覆盖那 16 字节），子固件
  一运行记录即失效，CRC 校验不过，快速引导静默回退常规路径 → 用户仍落到启动器。要让该
  选项生效，必须每个子固件都配合，正是开机策略规则所禁止的依赖；改写 otadata 无此依赖。

  （2）启动器的显示初始化补上深睡唤醒恢复：先解除子固件留下的引脚 hold（`hold` 可跨
  复位保留，不解除会让 SPI 初始化命令全部无效，表现为有背光但黑屏），并在面板复位前
  单独补发 `0x11` SLPOUT 以退出停振态（本板复位脚未接 MCU，停振态面板收 SWRESET 会
  死锁）。这一处兜住的是仍回退到启动器的路径，对任何使用深睡的子固件都有效。新增契约
  测试 `tests/test_display_wake_contract.py`（6 例）钉住唤醒恢复顺序与 otadata 续期接线，
  已纳入 `tools/validate.sh --static`。
